(() => {
  ...
  function formatProjectValue(label, value) {
    ...
    if (labelN.includes("PROYECTO OP")) {
      ...
      if (/^OP\s*/i.test(clean)) return `OP ${clean.replace(/^OP\s*/i, "")}`;
      return `OP ${clean}`;
    }
  ...
  function isProjectHeader(value) {
    const n = normalize(value);
    return n === "PROYECTO OP" || n === "PROYECTO" || n === "OBRA";
  }

  function isValidProjectValue(value) {
    const n = normalize(value);
    if (!n || isProjectHeader(n)) return false;

    // Evita confundir los encabezados vecinos con el valor del proyecto.
    const columnHeaders = [
      "CANTIDAD", "CANT", "CANT.", "QTY", "CONTRATISTA", "MARCA",
      "ITEM", "DESCRIPCION", "PROCESO", "HOJA", "FILA"
    ];
    if (columnHeaders.includes(n) || n.startsWith("CANTIDAD ")) return false;

    // Los códigos de esta columna pueden venir como "OP 08/25" o solo "08/25".
    return /^OP\s*[A-Z0-9][A-Z0-9./_-]*$/i.test(n) ||
      /^[A-Z0-9][A-Z0-9./_-]*$/i.test(n);
  }

  function projectBelowHeader(rows, headerRow, column) {
    const limit = Math.min(rows.length - 1, headerRow + 60);
    for (let r = headerRow + 1; r <= limit; r++) {
      const value = safeText(rows[r]?.[column]).trim();
      if (!value) continue;
      if (isProjectHeader(value)) break;
      if (isValidProjectValue(value)) return formatProjectValue("Proyecto OP", value);
    }
    return "";
  }
  ...
  function getProjectFromRow(row) {
    for (let i = 0; i < (row || []).length; i++) {
      ...
      // [Proyecto OP] [0524]
      if (isProjectHeader(n)) {
        const next = safeText(row[i + 1]).trim();
        if (isValidProjectValue(next)) return formatProjectValue(text, next);
      }
    }
    return "";
  }
  ...
  function inferProject(rows, blockStart) {
    // En la planilla real "Proyecto OP" es un encabezado de columna y el
    // código (por ejemplo OP 08/25) aparece debajo, no en la celda contigua.
    for (let r = blockStart; r >= Math.max(0, blockStart - 60); r--) {
      for (let c = 0; c < (rows[r] || []).length; c++) {
        if (!isProjectHeader(rows[r][c])) continue;
        const p = projectBelowHeader(rows, r, c);
        if (p) return p;
      }
    }
  ...
  function renderSearchResults(terms) {
    ...
    ["Proyecto OP", "Contratista", "Marca", "Proceso", "Cantidad"].forEach(label => {
      const chip = document.createElement("span");
      chip.className = "chip";
    ...
    const headers = ["Marca buscada", "Marca encontrada", "Proyecto OP", "Contratista", "Proceso", "Cantidad", "Hoja", "Descripción"];
    ...
    detailResults.forEach(item => {
      ...
      [
        ...
        item.role, formatNumber(item.quantity), item.sheet, item.description
      ].forEach((value, i) => {
  ...
  function renderAssignmentSummary() {
    ...
    ["Proyecto OP", "Contratista", "Proceso", "Cantidad asignada", "Marcas"].forEach(label => {
      const chip = document.createElement("span");
      chip.className = "chip";
      chip.textContent = label;
      fieldChips.appendChild(chip);
    ...
    const headers = ["Proyecto OP", "Contratista", "Proceso", "Cantidad asignada", "Marcas únicas", "Marcas"];
    const thead = document.createElement("thead");
    const hr = document.createElement("tr");
    headers.forEach(label => {
      const th = document.createElement("th");
  ...
  function exportCSV() {
    ...
    if (isSummary) {
      lines.push(["Proyecto OP", "Contratista", "Proceso", "Cantidad asignada", "Marcas únicas", "Marcas"].map(esc).join(","));
    ...
    lines.push(["Proyecto OP", "Contratista", "Marca", "Proceso", "Cantidad", "Hoja", "Descripción"].map(esc).join(","));
    detailResults.forEach(r => {
      lines.push([
        r.project, r.contractor, r.mark, r.role, r.quantity, r.sheet, r.description
      ].map(esc).join(","));
    });
